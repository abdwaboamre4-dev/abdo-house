 ...
 * Creative Performance & Digital Solutions Agency (Saudi Arabia, UAE, Qatar, Kuwait, Bahrain & GCC)
 */
...
class App {
  ...
  init() {
    ...
    // 8. Setup Interactive Tabbed Pricing Switcher
    this.initPricingTabs();

    // 9. Setup Touch-Optimized Accordion FAQ
    this.initFAQ();

    // 10. Setup Inquiry Drawer & WhatsApp Subscription Flow
    this.initInquiryDrawer();

    // 11. Setup Plan Selection Triggers
    this.initPlanSelectors();

    // 12. Setup Mobile Navigation
    this.initMobileMenu();
  }
  ...
  initScrollAnimations() {
    ...
    document.querySelectorAll('section, .project-card, .agency-service-card, .platform-card, .pricing-card-agency, .testimonial-card, .risk-reversal-banner').forEach(el => {
  ...
  initPlanSelectors() {
    ...
    planButtons.forEach(btn => {
      btn.addEventListener('click', (e) => {
        e.preventDefault();
        const planName = btn.dataset.plan || 'Sales Scale';
  ...
  initInquiryDrawer() {
    ...
    if (form) {
      form.addEventListener('submit', (e) => {
        ...
        const selectedPlan = planInput ? planInput.value : 'Sales Scale Accelerator ($890/mo)';
        ...
        submitBtn.innerHTML = '<span>جاري تجهيز بيانات الطلب...</span>';
        submitBtn.disabled = true;

        // Build elegant WhatsApp message text
        const waMessageLines = [
          "⚡ *ABDO HOUSE — AGENCY PROJECT INQUIRY*",
          "----------------------------------------",
          ...
          clientDesc || "مرحباً، أود بدء التعاون ومضاعفة مبيعات مشروعي مع ABDO HOUSE."
        ];
        ...
        setTimeout(() => {
          form.innerHTML = `
            <div style="text-align: center; padding: 3rem 1rem;">
              ...
              <h3 style="font-family: 'Cairo', var(--font-serif); font-size: 2.2rem; color: var(--text-primary); margin-bottom: 0.75rem;">تم استلام تفاصيل طلبك بنجاح</h3>
              <p style="color:var(--text-secondary); max-width:460px; margin:0 auto 2rem; line-height:1.7; font-size:0.95rem;">
                تم تجهيز ملخص الباقة <strong>${selectedPlan}</strong>. اضغط أدناه للانتقال للمحادثة المباشرة على واتساب مع فريق ABDO HOUSE.
              </p>
              
              <div style="display:flex; flex-direction:column; gap:1rem; max-width:380px; margin:0 auto;">
                <a href="${waUrl}" target="_blank" rel="noopener" class="btn btn-primary" style="width:100%; font-family: 'Cairo', sans-serif;">
                  <span>متابعة على واتساب &larr;</span>
                </a>
